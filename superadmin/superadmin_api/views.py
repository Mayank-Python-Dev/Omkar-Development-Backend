...
class InvestorsGalaDetailView(APIView):
    ...
    def get(self, request, uuid, *args, **kwargs):
        try:
            ...
            # print(get_investor_contract_qs)
        ...
        try:
            ...
            if get_user_type is not None:
                if get_user_type == "Farmer":
                    ...
                    queryset = ContractFarmer.objects.filter(user__groups__name = get_user_type).values("id","user__user_uid","user__username","user__email").order_by("id").distinct()
                    context = {
                        "status":status.HTTP_200_OK,
                        "success":True,
                        "response": queryset
                    ...
                    queryset = ContractInvestor.objects.filter(user__groups__name = get_user_type).values("id","user__user_uid","user__username","user__email").order_by("id").distinct()
                    context = {
                        "status":status.HTTP_200_OK,
                        "success":True,
                        "response": queryset
...
class FreeGalaDetailViewAPI(APIView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        serializer = InvestorRentalGalaDetailSerializer(get_contracts,many=True)
        context = {
                'status':status.HTTP_200_OK,
                'success':True,
                'response':serializer.data,
